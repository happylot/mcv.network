...
| Yêu cầu | Mục tiêu |
|----------|----------|
| Kích thước SDK (Web) | < 8KB gzipped |
...
│                                                                     │
│  ┌──────────────┐  ┌──────────────┐  ┌────────────────────────────┐│
│  │ /v1/ad       │  │ /v1/event    │  │ /v1/config                ││
│  │ (Ad Request) │  │ (Tracking)   │  │ (SDK Config/A-B)          ││
│  └──────┬───────┘  └──────┬───────┘  └────────────┬───────────────┘│
└─────────┼──────────────────┼───────────────────────┼───────────────┘
          │                  │                       │
          ▼                  ▼                       ▼
┌─────────────────┐  ┌──────────────┐  ┌────────────────────────────┐
│  AD DECISION    │  │  EVENT       │  │  CONFIG SERVICE            │
│  ENGINE         │  │  PIPELINE    │  │  (Feature flags, A/B)      │
│                 │  │              │  │                            │
│  • Targeting    │  │  • Kafka     │  └────────────────────────────┘
│  • Auction      │  │  • Flink     │
│  • Pacing       │  │  • ClickHouse│
...
│   ├── identity.ts        — First-party ID, fingerprint (privacy-safe)
...
│   ├── auction.ts         — Client-side header bidding (Prebid lite)
...
```
Region: US-East-1 (primary), AP-Southeast-1 (SEA traffic)

├── CloudFront CDN
│   ├── SDK JS files (edge-cached, 1yr TTL)
│   ├── Ad creative assets (images, video)
│   └── Edge compute (Lambda@Edge for geo/device detection)
│
├── EKS Cluster (Ad Server)
│   ├── Ad Decision Service (Go, 50 pods, <20ms p99)
│   ├── Auction Service (Rust, 20 pods, <10ms p99)
│   ├── Pacing Service (Go, 10 pods)
│   └── Fraud Detection (Python, 30 pods)
│
├── Data Pipeline
│   ├── Kafka (MSK) — 100K events/sec throughput
│   ├── Flink — Real-time aggregation
│   └── ClickHouse — Analytics warehouse
│
├── Storage
│   ├── RDS (PostgreSQL) — Campaign configs, billing
│   ├── ElastiCache (Redis) — Session, frequency caps, user features
│   ├── S3 — Creative storage, logs, ML model artifacts
│   └── DynamoDB — User ID graph (low-latency lookups)
│
└── ML Platform
    ├── SageMaker — Model training (CTR, CVR prediction)
    ├── Model Registry — Version management
    └── Feature Store — Real-time feature serving
```

### 8.3 Estimated Monthly Infrastructure Cost

| Component | Specs | Cost/tháng |
|-----------|-------|-----------|
| CloudFront CDN | 10TB transfer | $850 |
| EKS (Ad Server) | 50 pods, c6g.xlarge | $4,500 |
| Kafka (MSK) | 6 brokers | $2,000 |
| ClickHouse | 3 nodes, r6g.2xlarge | $2,500 |
| RDS PostgreSQL | db.r6g.xlarge, multi-AZ | $800 |
| Redis (ElastiCache) | r6g.xlarge, 3 nodes | $1,200 |
| S3 + DynamoDB | 5TB + 100K RCU | $600 |
| SageMaker | Training + inference | $1,500 |
| **Tổng (Phase 1)** | | **~$14,000/tháng** |
| **Tổng (Scale - Phase 3)** | | **~$45,000/tháng** |
...
|-----------|----------|-------------|
| **M1 — Alpha** | Tuần 1-4 | Web SDK core (loader + native widget + tracking) |
| **M2 — Beta Web** | Tuần 5-8 | All web formats, AdX/Prebid integration, self-serve portal MVP |
| **M3 — Mobile Alpha** | Tuần 6-10 | Android SDK (native + banner + rewarded) |
| **M4 — iOS + Airtalk** | Tuần 8-12 | iOS SDK, Airtalk app integration, rewards system |
| **M5 — Production** | Tuần 12-14 | Full QA, load testing, security audit, documentation |
| **M6 — Scale** | Tuần 14-18 | ML targeting, RTB engine, commerce formats |
...
|------------|---------|----------------------|
| Google Ad Manager | Ad serving (Phase 1) | Custom ad server (Phase 3) |
| Prebid.js | Header bidding | Custom auction engine |
| IAS / MOAT | Viewability verification | In-house solution |
